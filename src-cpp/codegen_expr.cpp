...
TypeAnnotation CodeGen::resolve_expr_type(Expr *expr) {
  ...
  if (auto *deref = dynamic_cast<DerefExpr *>(expr)) {
    ...
    if (base.pointer_depth > 0) { base.pointer_depth--; return base; }
    return {TypeKind::Void};
  }
  if (auto *sub = dynamic_cast<SubscriptExpr *>(expr)) {
    TypeAnnotation base = resolve_expr_type(sub->array.get());
    if (base.array_size > 0) { base.array_size = 0; return base; }
    if (base.kind == TypeKind::Slice) return base.tuple_types[0];
    if (base.pointer_depth > 0) { base.pointer_depth--; return base; }
    return {base.kind};
  }
  if (auto *bin = dynamic_cast<BinaryExpr *>(expr)) {
    ...
    switch (bin->op) {
      ...
      default:
        return (l.kind != TypeKind::Void) ? l : TypeAnnotation{TypeKind::Int64};
    }
  }
  ...
  if (auto *ctor = dynamic_cast<ConstructorExpr *>(expr)) {
    for (auto &[ename, _] : enum_types) {
      ...
      if (vi >= 0) return {TypeKind::Enum, 0, 0, ename};
    }
  ...
  if (auto *atm = dynamic_cast<AtomicExpr *>(expr)) {
    if (atm->op == AtomicOp::Fence) return {TypeKind::Void};
    if (!atm->args.empty()) {
      TypeAnnotation ptr_ann = resolve_expr_type(atm->args[0].get());
      if (ptr_ann.pointer_depth > 0) { ptr_ann.pointer_depth--; return ptr_ann; }
  ...
  if (auto *call = dynamic_cast<CallExpr *>(expr)) {
    if (call->callee_expr) {
      ...
      if (callee_ann.kind == TypeKind::Fn && !callee_ann.tuple_types.empty())
    ...
    if (f) {
      ...
      if (ret_type->isStructTy()) {
        if (auto *st = dyn_cast<StructType>(ret_type); st && st->hasName())
...
Value *CodeGen::get_lvalue_ptr(Expr *expr, Type **out_type) {
  ...
  if (auto *sub = dynamic_cast<SubscriptExpr *>(expr)) {
    ...
    if (base_ann.array_size > 0) {
      ...
      Value *indices[] = { ConstantInt::get(Type::getInt64Ty(Context), 0), index };
  ...
  if (auto *field = dynamic_cast<FieldAccessExpr *>(expr)) {
    ...
    if (base_ann.kind == TypeKind::Tuple) {
      ...
      if (out_type && idx < (int)base_ann.tuple_types.size())
...
// =========================================================================
// Extracted expression evaluation sub-handlers
// =========================================================================
...
Value *CodeGen::eval_constructor(ConstructorExpr *ctor, Type *expected_type) {
  ...
  for (auto &[ename, st] : enum_types) {
    int variant_idx = get_enum_variant_index(ename, ctor_var_short);
    if (variant_idx < 0) continue;

    Value *result = UndefValue::get(st);
    Type *tag_type = st->getElementType(0);
    result = Builder.CreateInsertValue(result, ConstantInt::get(tag_type, variant_idx), {0});
    StructType *var_type = cast<StructType>(st->getElementType(1 + variant_idx));
    ...
    for (size_t fi = 0; fi < ctor->fields.size(); fi++) {
      ...
      std::string struct_key = ename + "::" + ctor_var_short;
      if (field_name.empty()) {
        actual_idx = (int)fi;
      ...
      var_data = Builder.CreateInsertValue(var_data, fv, {(unsigned)actual_idx});
    }
  ...
  if (st_it != struct_types.end()) {
    ...
    for (size_t fi = 0; fi < ctor->fields.size(); fi++) {
      ...
      Value *fv = eval_expr(field_expr.get(), field_type);
      if (!fv) return nullptr;
      result = Builder.CreateInsertValue(result, fv, {(unsigned)actual_idx});
    }
    return result;
  }
...
Value *CodeGen::eval_match(MatchExpr *match, Type *expected_type) {
...
Value *CodeGen::eval_binary(BinaryExpr *bin, Type *expected_type) {
  ...
  if ((bin->op == BinOp::Add || bin->op == BinOp::Sub) &&
    ...
    if (l->getType()->isPointerTy() && !r->getType()->isPointerTy()) {
      ...
      if (ann.pointer_depth > 0) { ann.pointer_depth--; pointee = get_llvm_type(ann); }
    ...
    if (ptr_val && idx_val)
      return Builder.CreateGEP(pointee ? pointee : Type::getInt8Ty(Context),
                               ptr_val, idx_val, "ptr.offset");
  }
...
Value *CodeGen::eval_closure(ClosureExpr *closure, Type *expected_type) {
  ...
  fn_param_types.push_back(PointerType::getUnqual(Context));
  for (auto &p : closure->params)
  ...
  BasicBlock *saved_insert_bb = Builder.GetInsertBlock();
  {
    ...
    for (size_t ci = 0; ci < captures.size(); ci++) {
      ...
      if (needs_type_annotation(cap_anns[ci].kind, cap_anns[ci].pointer_depth,
                                cap_anns[ci].array_size))
  ...
  for (size_t ci = 0; ci < captures.size(); ci++) {
    ...
    if (cap_val_it != named_values.end()) {
      Value *cap_val = Builder.CreateLoad(cap_types[ci], cap_val_it->second, captures[ci]);
...
Value *CodeGen::eval_call(CallExpr *call, Type *expected_type) {
  // Expression-based call (closure variable / lambda call)
  if (call->callee_expr) {
    TypeAnnotation callee_ann = resolve_expr_type(call->callee_expr.get());
    ...
    Value *fn_i8 = Builder.CreateLoad(PointerType::getUnqual(Context), fn_ptr_ptr, "fn_ptr");
  ...
  if (!callee) {
    ...
    if (current_bb) {
      ...
      if (colon != std::string::npos) {
        callee = M.getFunction(cur_fn_name.substr(0, colon) + "::" + actual_callee);
      }
    }
  }
  if (!callee) {
    ...
    if (fnv_it != named_values.end() && fnt_it != named_type_anns.end() &&
      ...
      Type *callee_llvm = get_llvm_type(callee_ann);
      if (!callee_llvm) callee_llvm = PointerType::getUnqual(Context);
      Value *closure_ptr = Builder.CreateLoad(callee_llvm, fnv_it->second, actual_callee);
      if (!closure_ptr) return nullptr;
  ...
  for (size_t i = 0; i < call->args.size(); i++) {
    ...
    if (i < fixed_params) {
      ...
      param_type = dynamic_cast<StringExpr *>(call->args[i].get())
                       ? static_cast<Type *>(PointerType::getUnqual(Context))
                       : Type::getInt64Ty(Context);
    }
...
Value *CodeGen::eval_method_call(MethodCallExpr *mcall, Type *expected_type) {
  TypeAnnotation obj_type = resolve_expr_type(mcall->object.get());
  std::string type_name;
  if (obj_type.kind == TypeKind::Struct || obj_type.kind == TypeKind::Enum)
    type_name = obj_type.struct_name;
...
Value *CodeGen::eval_atomic(AtomicExpr *atm) {
...
Value *CodeGen::eval_compound_assign(CompoundAssignExpr *compound) {
  ...
  switch (compound->op) {
    ...
    case BinOp::Div:    result = is_unsigned ? Builder.CreateUDiv(current, rhs)
                                             : Builder.CreateSDiv(current, rhs); break;
    case BinOp::Mod:    result = is_unsigned ? Builder.CreateURem(current, rhs)
                                             : Builder.CreateSRem(current, rhs); break;
...
Value *CodeGen::eval_if_expr(IfExpr *ifexpr, Type *expected_type) {
  Function *fn = Builder.GetInsertBlock()->getParent();
...
Value *CodeGen::eval_expr(Expr *expr, Type *expected_type) {
  ...
  if (auto *ctor = dynamic_cast<ConstructorExpr *>(expr))
    return eval_constructor(ctor, expected_type);
  ...
  if (auto *borrow = dynamic_cast<BorrowExpr *>(expr)) {
    // &fn_name — function pointer value
  ...
  if (auto *match = dynamic_cast<MatchExpr *>(expr))
    return eval_match(match, expected_type);
  ...
  if (auto *un = dynamic_cast<UnaryExpr *>(expr)) {
    ...
    switch (un->op) {
      case UnaryOp::BitNot: return Builder.CreateNot(op);
      case UnaryOp::Neg:
      default:
        return (expected_type && expected_type->isFPOrFPVectorTy())
                   ? Builder.CreateFNeg(op) : Builder.CreateNeg(op);
    }
  }

  if (auto *bin = dynamic_cast<BinaryExpr *>(expr))
    return eval_binary(bin, expected_type);

  if (auto *closure = dynamic_cast<ClosureExpr *>(expr))
    return eval_closure(closure, expected_type);

  if (auto *call = dynamic_cast<CallExpr *>(expr))
    return eval_call(call, expected_type);

  if (auto *mcall = dynamic_cast<MethodCallExpr *>(expr))
    return eval_method_call(mcall, expected_type);

  if (auto *atm = dynamic_cast<AtomicExpr *>(expr))
    return eval_atomic(atm);
  ...
  if (auto *ifexpr = dynamic_cast<IfExpr *>(expr))
    return eval_if_expr(ifexpr, expected_type);
